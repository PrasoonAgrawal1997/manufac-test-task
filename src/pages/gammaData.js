...
const GammaData = (props) => {
  const { numberOfClasses } = props;
  const [arrToCalculate, setArrToCalculate] = useState([]);
  const [tableData, setTableData] = useState([]);

  // it is the useEffect used to calculate all the gamma value of particular class, and store it in an array
  ...
  // it is the useEffect, used to find mean, median, mode of every class gamma, and store it in an array
  ...
  return (
    <div>
      <div className="header-text">Gamma Data</div>
